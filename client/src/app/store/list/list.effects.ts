...
import { addList, addListSuccess, loadList, loadListSuccess } from "./list.actions";
...
export class ListEffects{
    constructor(private action$: Actions, private listCardsService: ListsCardsService){}
    ...
    this.action$.pipe(
        ...
        switchMap(action =>
            this.listCardsService.createListCards(action.listinput).pipe(
                switchMap(data => of(
                    addListSuccess({ listInput: data as CreateListCardsModel }),
                )),
